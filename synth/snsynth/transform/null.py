...
class NullTransformer():
    """Transformer for data that contains Null values. Modified to take data as an np.array instead of a pd series.
    ...
    def _get_missing_value_replacement(self, data):
        ...
        if self._missing_value_replacement in {'mean', 'mode'} and np.isnan(data).all():
        ...
        if self._missing_value_replacement == 'mode':
            return mode(data[~np.isnan(data)])[0][0]
    ...
    def fit(self, data):
        ...
        if not self.nulls and self._model_missing_values:
            ...
            guidance_message = (
                f'Guidance: There are no missing values in column. '
                'Extra column not created.'
            )
            LOGGER.info(guidance_message)

        if not self._model_missing_values:
            self._null_percentage = np.sum(null_values) / len(data)
    ...
    def transform(self, data):
        ...
        if isna.any() and self._missing_value_replacement is not None:
            data[isna] = self._missing_value_replacement

        if self._model_missing_values:
            null_indicator = isna.astype(np.float64)
            return np.concatenate([data.reshape(-1, 1), null_indicator.reshape(-1, 1)], axis=1)

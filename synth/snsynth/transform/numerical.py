...
class ClusterBasedNormalizer(FloatFormatter):
    ...
    # Overwrite the transform method in rdt to allow using non-dataframe
    def transform(self, val):
        return self._transform(val)
        
    # Overwrite the transform method in rdt to allow using non-dataframe
    def reverse_transform(self, val):
        return self._reverse_transform(val)
        
    # Overwrite the same method in rdt to handle np.array
    def _validate_values_within_bounds(self, data):
        if self.computer_representation != 'Float':
